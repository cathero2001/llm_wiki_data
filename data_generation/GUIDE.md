...
# 3. 导入 llm_wiki
#    将 output/phase_mX/ 目录下的文件导入 llm_wiki（手动 Import 按钮）
```
...
**导入方式**：在 llm_wiki 界面点击 Import，选中 `output/phase_mX/` 下的所有文件导入（无需 sources/ 子目录）。
...
| M6 | 22 | 上线通知、恒通物流新客户、青云运营反馈、**性能基准v3** |
...
**输出内容：**
- `output/phase_m1/` 到 `output/phase_m6/` — 各个 phase 单独分批
...
| 导入顺序 | AI 新增能力 |
|---------|-------------|
| 先导入 `output/phase_m1/` 下的文件 | 知道项目启动、技术选型讨论中（Pinecone/Milvus/FAISS 三选一） |
| 再导入 `output/phase_m2/` 下的文件 | 知道 Pinecone 成本超支改 FAISS、技术方案已冻结 |
| 再导入 `output/phase_m3/` 下的文件 | 知道 FAISS 私有化有隐患、Milvus 内存问题、MVP 开发中 |
| 再导入 `output/phase_m4/` 下的文件 | 知道切换 Milvus、内测 Bug、性能 v1 未达标 |
| 再导入 `output/phase_m5/` 下的文件 | 知道资源争抢决策、性能 v2 达标、青云签约 |
| 再导入 `output/phase_m6/` 下的文件 | 完整项目周期，性能 v3 超预期，所有决策链路清晰 |
...
| 占位符 | 原始值（anchor_date=2026-04-27） | 说明 |
|--------|--------------------------------|------|
| `{{date_0}}` | 2026-03-16 | project_start，第 0 天 |
| `{{date_7}}` | 2026-03-23 | project_start + 7 天 |
| `{{date_14}}` | 2026-03-30 | project_start + 14 天 |
| `{{date_21}}` | 2026-04-06 | project_start + 21 天 |
| `{{date_28}}` | 2026-04-13 | project_start + 28 天 |
| `{{date_35}}` | 2026-04-20 | project_start + 35 天 |
| `{{date_42}}` | 2026-04-27 | anchor_date 当天 |
...
锚定日期：`2026-04-27`（anchor_date 当天）
